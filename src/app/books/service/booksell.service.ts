...
export class BookSellService {
  ...
  booksSoldByUser(id_user: number, page: number): Observable<any> {
    return this.httpClient.get<any>(environment.API + this.API + '/user/' + id_user + '/page/' + page);
  }

  userPurchasedBooks(id_user: number, id_book: number): Observable<number> {
    return this.httpClient.get<any>(environment.API + this.API + '/user/' + id_user + '/book/' + id_book);
  }
}
